...
import { getArtistSearchStart,getTrackStart } from '../../store/slices/musicSlice';
...
function Artist() {
    ...
    useEffect(() => {
        // getAllMusicList();
        dispatch(getArtistSearchStart({"artist":state.artist.name}))
      }, []);
  ...
  return (
    ...
    <Card orientation="horizontal"  variant="outlined" sx={{minWidth: 100,backgroundColor:"transparent"}} className='artistcontainer' >
            <img
              src={`${state.artist.src}?h=120&fit=crop&auto=format`}
              srcSet={`${state.artist.src}?h=120&fit=crop&auto=format&dpr=2 2x`}
              alt={state.artist.title}
              width="20%"
            />
        </Card>
